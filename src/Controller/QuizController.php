...
//    #[Route('/quiz/{userId}/{categoryId}/resultsFinal', name: 'app_results_final')]
// public function finalResults(ScoreRepository $scoreRepository): Response
// {
//     // Récupérer les scores les plus élevés pour chaque quiz pour tous les utilisateurs
//     $highestScoresByQuiz = $scoreRepository->findHighestScoresByQuiz();

//     // Structure pour stocker la somme des scores les plus élevés par quiz
//     $quizTotalScores = [];

//     // Calculer la somme des scores les plus élevés par quiz
//     foreach ($highestScoresByQuiz as $score) {
//         $quizId = $score['quizId'];
//         if (!isset($quizTotalScores[$quizId])) {
//             $quizTotalScores[$quizId] = [
//                 'quizId' => $quizId,
//                 'totalHighestScore' => 0,
//                 'users' => [],
//             ];
...
//         $quizTotalScores[$quizId]['totalHighestScore'] += $score['highestScore'];

//         // Ajouter chaque utilisateur avec son score le plus élevé pour ce quiz
//         $quizTotalScores[$quizId]['users'][] = [
//             'userId' => $score['userId'],
//             'prenom' => $score['prenom'],
//             'nom' => $score['nom'],
...
//     // Trouver le gagnant pour chaque quiz en fonction du score total le plus élevé
//     $winners = [];
//     foreach ($quizTotalScores as $quizScore) {
//         $maxScore = 0;
//         $winner = null;
//         foreach ($quizScore['users'] as $userScore) {
//             if ($userScore['highestScore'] > $maxScore) {
//                 $maxScore = $userScore['highestScore'];
//                 $winner = $userScore;
//             }
//         }
//         $winners[] = [
...
//             'winner' => $winner,
//             'totalHighestScore' => $quizScore['totalHighestScore'],
//         ];
//     }
